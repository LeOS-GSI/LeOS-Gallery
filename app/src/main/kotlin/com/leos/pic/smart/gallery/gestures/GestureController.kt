...
class GestureController(private val targetView: View) : View.OnTouchListener {
    companion object {
        private const val STATE_CHANGE_IDLE_DURATION =
            200L     // refocus on the image at dragging too, if the finger hasnt moved for this long
    ...
    private fun onScroll(e1: MotionEvent, e2: MotionEvent, dx: Float, dy: Float): Boolean {
        ...
        if (!isScrollDetected) {
            isScrollDetected =
                abs(e2.x - e1.x) > touchSlop || abs(e2.y - e1.y) > touchSlop
    ...
    private fun onFling(vx: Float, vy: Float): Boolean {
        ...
        flingScroller.fling(
            state.x.roundToInt(), state.y.roundToInt(),
