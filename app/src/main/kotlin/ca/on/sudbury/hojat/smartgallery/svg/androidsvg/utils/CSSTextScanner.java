...
public class CSSTextScanner extends TextScanner {
    static final java.util.regex.Pattern PATTERN_BLOCK_COMMENTS = java.util.regex.Pattern.compile("(?s)/\\*.*?\\*/");
     ...
     */
    public List<ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSParser.Selector> nextSelectorGroup() throws CSSParseException {
        if (empty())
            return null;

        java.util.ArrayList<ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSParser.Selector> selectorGroup = new java.util.ArrayList<>(1);
    ...
    private ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSTextScanner.AnPlusB nextAnPlusB() {
        if (empty())
            return null;
        ...
        ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSTextScanner.AnPlusB result;
        if (consume("odd"))
            result = new ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSTextScanner.AnPlusB(2, 1);
        else if (consume("even"))
            result = new ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSTextScanner.AnPlusB(2, 0);
        else {
            ...
            IntegerParser a = null,
                    b = IntegerParser.parseInt(input, position, inputLength);
            if (b != null)
            ...
            result = new ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSTextScanner.AnPlusB((a == null) ? 0 : aSign * a.value(),
     ...
     * Eg. for :lang(en)
     */
    private java.util.List<String> nextIdentListParam() {
        if (empty())
            return null;
     ...
     */
    private java.util.List<ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSParser.Selector> nextPseudoNotParam() throws CSSParseException {
        if (empty())
            return null;
        ...
        java.util.List<ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSParser.Selector> result = nextSelectorGroup();
...
//         int     pseudoStart = position;
        ...
        switch (identEnum) {
            ...
            case nth_last_of_type:
                ...
                ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSTextScanner.AnPlusB ab = nextAnPlusB();
                if (ab == null)
            ...
            case not:
                java.util.List<ca.on.sudbury.hojat.smartgallery.svg.androidsvg.utils.CSSParser.Selector> notSelectorGroup = nextPseudoNotParam();
            ...
            case lang:
                java.util.List<String> langs = nextIdentListParam();
