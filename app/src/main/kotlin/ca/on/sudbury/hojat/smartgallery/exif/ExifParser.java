...
class ExifParser {
    ...
    private final java.util.TreeMap<Integer, Object> mCorrespondingEvent = new java.util.TreeMap<Integer, Object>();
    ...
    private java.util.List<ExifParser.Section> mSections;
    ...
    private ExifParser(java.io.InputStream inputStream, int options, ExifInterface iRef) throws java.io.IOException, ExifInvalidFormatException {
        if (inputStream == null) {
            throw new java.io.IOException("Null argument inputStream to ExifParser");
        }

        android.util.Log.v(TAG, "Reading exif...");
        mSections = new java.util.ArrayList<ExifParser.Section>(0);
    ...
    private final java.nio.ByteBuffer mByteBuffer = ByteBuffer.wrap(mByteArray);
    ...
    private CountedDataInputStream seekTiffData(InputStream inputStream) throws IOException, ExifInvalidFormatException {
        ...
        while (true) {
            ...
            if (a > 10) {
                Timber.w("Extraneous " + (a - 1) + " padding bytes before section " + marker);
            }

            ExifParser.Section section = new ExifParser.Section();
            section.type = marker;
                ...
                case JpegHeader.TAG_M_EXIF:
                    if (itemlen >= 8) {
                        ...
                        if (header == EXIF_HEADER && headerTail == EXIF_HEADER_TAIL) {
                            tiffStream = new CountedDataInputStream(new java.io.ByteArrayInputStream(data, 8, itemlen - 8));
                            tiffStream.setEnd(itemlen - 6);
                            ignore = false;
                        } else {
                            Timber.v("Image cotains XMP section");
                        }
                    }
                    break;

                default:
                    android.util.Log.w(TAG, "Unknown marker: " + String.format("0x%2X", marker) + ", length: " + itemlen);
                    break;
            }
            ...
            if (!ignore) {
                ...
                android.util.Log.v(TAG, "ignoring marker: " + String.format("0x%2X", marker) + ", length: " + itemlen);
            }
        }
    }
    ...
    protected int next() throws IOException, ExifInvalidFormatException {
        ...
        while (mCorrespondingEvent.size() != 0) {
            ...
            try {
                ...
                android.util.Log.w(TAG, "Failed to skip to data at: " + entry.getKey() +
    ...
    private void skipTo(int offset) throws java.io.IOException {
    ...
    private void registerUncompressedStrip(int stripIndex, long offset) {
        mCorrespondingEvent.put((int) offset, new ExifParser.ImageEvent(EVENT_UNCOMPRESSED_STRIP, stripIndex));
    }
            ...
            if (mCorrespondingEvent.size() > 0) {
                if (mCorrespondingEvent.firstEntry().getKey() < mTiffStream.getReadByteCount() + size) {
                    ...
                    if (event instanceof ExifParser.ImageEvent) {
                        ...
                        Timber.w("Thumbnail overlaps value for tag: %s", tag);
                        Map.Entry<Integer, Object> entry = mCorrespondingEvent.pollFirstEntry();
                        Timber.w("Invalid thumbnail offset: %s", entry.getKey());
                    } else {
                        // Tag value overlaps another tag, shorten count
                        if (event instanceof ExifParser.IfdEvent) {
                            android.util.Log.w(TAG, "Ifd " + ((ExifParser.IfdEvent) event).ifd + " overlaps value for tag: \n" + tag);
                        } else if (event instanceof ExifParser.ExifTagEvent) {
                            android.util.Log.w(TAG, "Tag value for tag: \n" + ((ExifParser.ExifTagEvent) event).tag.toString() + " overlaps value for tag: \n" + tag);
                        }
                        size = mCorrespondingEvent.firstEntry().getKey() - mTiffStream.getReadByteCount();
                        android.util.Log.w(TAG, "Invalid size of tag: \n" + tag + " setting count to: " + size);
