...
package ca.on.sudbury.hojat.smartgallery.recyclerviewfastscroller
...
class RecyclerViewFastScroller @JvmOverloads constructor(
    ...
    private object Defaults {
        ...
        const val handleHeight: Int = R.dimen.default_handle_height
        const val handleWidth: Int = R.dimen.default_handle_width
    ...
    private fun updateTextInPopup(position: Int) {
        if (position == currentPopupItemPosition || position !in 0 until (recyclerView.adapter?.itemCount
                ?: 1)
        ) {
            return
        }
