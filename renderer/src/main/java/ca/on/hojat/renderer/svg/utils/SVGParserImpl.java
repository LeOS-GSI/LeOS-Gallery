...
class SVGParserImpl implements SVGParser {
    ...
    private void line(org.xml.sax.Attributes attributes) throws SVGParseException {
        debug("<line>");
    ...
    private void parseAttributesTextPosition(TextPositionedContainer obj, org.xml.sax.Attributes attributes) throws SVGParseException {
        for (int i = 0; i < attributes.getLength(); i++) {
            String val = attributes.getValue(i).trim();
            switch (SVGAttr.fromString(attributes.getLocalName(i))) {
                case x:
