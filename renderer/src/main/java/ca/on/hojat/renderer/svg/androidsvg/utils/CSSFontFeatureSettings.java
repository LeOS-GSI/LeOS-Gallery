...
public class CSSFontFeatureSettings {
    ...
    private final java.util.HashMap<String, Integer> settings;
    ...
    static CSSFontFeatureSettings parseFontFeatureSettings(String val) {
        ...
        while (!scan.empty()) {
            CSSFontFeatureSettings.FontFeatureEntry entry = nextFeatureEntry(scan);
            if (entry == null)
                return null;
    ...
    private static CSSFontFeatureSettings.FontFeatureEntry nextFeatureEntry(TextScanner scan) {
        scan.skipWhitespace();
        ...
        return new CSSFontFeatureSettings.FontFeatureEntry(name, val);
    }
    ...
    private static java.util.List<String> extractTokensAsList(String val) {
        TextScanner scan = new TextScanner(val);
        scan.skipWhitespace();
        if (scan.empty())
            return null;
    ...
    // Used only by parseFontVariant()
    // Only looks for the values unique to this property
    private static CSSFontFeatureSettings parseVariantCapsSpecial(java.util.List<String> tokens) {
        CSSFontFeatureSettings result = new CSSFontFeatureSettings(CAPS_ALL_OFF);
        ...
        if (Objects.equals(which, TOKEN_ERROR))
            return ERROR;
        if (which == null)
            return null;
